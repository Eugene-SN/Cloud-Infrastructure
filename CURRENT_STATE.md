...
Mattermost public ingress is **ACCEPTED**: `STAGE4E_MATTERMOST_INGRESS_ACCEPTANCE=PASS`. Native application configuration context audit is also complete: first System Admin account exists and System Console access was verified; current runtime has `SiteURL` unset, bot account creation disabled, public user creation enabled, push notifications disabled/unconfigured, Calls plugin enabled, `mmctl 11.11.0` local mode operational, and Calls plugin ID `com.mattermost.calls`. Marker: `STAGE4E_MATTERMOST_NATIVE_CONFIG_CONTEXT=PASS`. `https://chat.escloud.us` now serves Mattermost through the existing Xray -> host nginx -> loopback backend path, WebSocket `/api/v4/websocket` returns `101 Switching Protocols`, Authelia is absent by design, and n8n/CloudCLI/mail ingress non-regression passed. Acceptance record: `STAGE_04E_MATTERMOST_INGRESS_ACCEPTANCE_2026-09-18.md`.
