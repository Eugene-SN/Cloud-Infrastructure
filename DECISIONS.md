...
## 2026-09-21 — Stage 7B manual-only enforcement and deployment cleanup

**Status:** COMPLETE / ACCEPTED

**Context:** Final Stage 7B audit confirmed that Ubuntu periodic APT and
unattended-upgrade paths were enabled despite the accepted manual-only update
contract. It also found inactive deployment payload, a duplicate status
renderer, generated bytecode/test artifacts, unused Docker update residue and a
stale Stage 4 scratch worktree.

**Decision:**
- install the canonical late-sorting APT override as
  `/etc/apt/apt.conf.d/99-edge-maintenance-manual-only`, with all periodic
  actions set to `0`;
- mask and stop `apt-daily.timer`, `apt-daily.service`,
  `apt-daily-upgrade.timer`, `apt-daily-upgrade.service` and
  `unattended-upgrades.service` while preserving explicit `APT_EDGE` calls;
- use only `/opt/edge-maintenance/scripts/maintenance-status-render` from both
  direct and Semaphore Refresh paths and remove the `/usr/local/bin` duplicate;
- remove inactive `/opt` playbooks/dashboard/installer payload, generated test
  artifacts, bytecode caches, the unused anonymous volume and unused Docker
  images;
- retain Semaphore-managed Git checkouts and intentional recovery backups;
- archive the unique Stage 4 scratch material before removing its invalid
  workspace copy.

**Verification:** all five autonomous APT unit paths are masked/inactive; the
effective APT periodic values are all `0`; a post-cleanup Refresh passed with 16
actionable targets, no unresolved/check-failed state and no pending reboot; the
Master health gate, nginx validation, systemd health and canonical contract
tests passed; exactly one runtime `actions.json` remains; Docker has six active
images for six running containers, zero unused images and zero local volumes.

**Cleanup evidence:** 51 temporary entries were removed; Docker reclaimed about
2.39 GB; the Stage 4 archive is
`/srv/backups/edge-stage4-work-final-20260921T1907.tar.gz` with SHA256
`8e7f164526e0916ce0876b49ef51bb6609a0eef1553d366ed06c6e9470a88050`.

**Acceptance marker:** `STAGE07B_MANUAL_ONLY_CLEANUP=PASS`.

**Supersedes:** the unattended-upgrade blocker and incomplete Stage 7B status
recorded after Master Batch and generated-action acceptance. Stage 7B is now
...
## 2026-09-25T00:04:00+03:00 — Nextcloud latest-stable discovery and upstream Docker update architecture

**Status:** ACCEPTED

**Context:** The accepted Version Discovery v2 implementation treated the Docker alias `nextcloud:stable-apache` as the authority for the current stable Nextcloud Server version. Production Nextcloud `34.0.4` on its default `stable` release channel reported `35.0.1` as available, while `stable-apache` still resolved to the 34 branch. Official Nextcloud documentation confirms that the application release channel and Docker image aliases are distinct concerns: Docker deployments are updated by pulling/recreating the image, while Nextcloud major upgrades must follow the supported sequential upgrade procedure and must be repeated until the highest available/applicable release is reached.
...
## 2026-09-25 — Antigravity native auto-update ownership

**Status:** ACCEPTED

**Context:** Runtime audit confirmed that Antigravity CLI has its upstream-native background self-updater enabled by default on edge: `AGY_CLI_DISABLE_AUTO_UPDATE` is absent and the native updater state directory exists. At the same time, Antigravity remained a manual Maintenance target using `agy update`, creating duplicate update ownership.
