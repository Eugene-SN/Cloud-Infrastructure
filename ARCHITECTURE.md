...
Stage 07.2 supersedes the Stage 7B blanket manual-only ownership rule with a native-first hybrid contract. `update.escloud.us` remains the sole operator surface for Maintenance-owned manual updates, while supported upstream-native automatic lifecycles remain authoritative and are monitor-only in Maintenance. Current native owners are Codex managed-daemon auto-update, Hermes native cron + settlement, and Ubuntu security updates through package-owned unattended-upgrades. Semaphore has no autonomous schedules. The manual target set is 18 units; Codex and Hermes are explicitly non-actionable status rows.
