...
`update.escloud.us` remains separate from `app.escloud.us`. It is the single Stage 7 operator origin: `/status/` is the custom maintenance dashboard and `/project/1/history` is the full Semaphore UI. No separate `ops.escloud.us` application endpoint is required.
